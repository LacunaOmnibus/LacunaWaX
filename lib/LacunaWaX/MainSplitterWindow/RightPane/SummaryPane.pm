...
package LacunaWaX::MainSplitterWindow::RightPane::SummaryPane {
    ...
    ### CHECK - should change these to "body_(name|id)".  They're going to be 
    ### set whether we're a station or a planet.
    ...
    sub _build_planet_id {#{{{
        ...
        my $id;
        if( wxTheApp->game_client->isa_planet($self->planet_name) ) {
            $id = wxTheApp->game_client->planet_id( $self->planet_name );
        }
        elsif( wxTheApp->game_client->isa_station($self->planet_name) ) {
            $id = wxTheApp->game_client->station_id( $self->planet_name );
        }
        else {
            die "Got bogus body name " . $self->planet_name;
        }
...
Orbit $s->{orbit} around $s->{star_name} (ID $s->{star_id}), in zone $s->{zone}\n";
        ...
        if( $self->type eq 'Station' ) {
            ...
            if( @laws ) {
                $text .= "\nLaws:\n";
                $text .= "-------------------------------\n";
                foreach my $l(sort {$a->{'name'} cmp $b->{'name'}}@laws) {
                    $text .= "$l->{'name'}\n";
                }
            }
        }
